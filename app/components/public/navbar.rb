...
module Components
  module Public
    class Navbar < Components::Base
        ...
        header(class: "supports-backdrop-blur:bg-background/80 sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-2xl backdrop-saturate-200") do
          div(class: "px-2 sm:px-4 sm:container flex h-14 items-center justify-between") do
            div(class: "mr-4 flex items-center") do
              ...
              # Link(href: root_path, variant: :ghost, class: "hidden md:inline-block") { "Docs" }
              # Link(href: root_path, variant: :ghost, class: "hidden md:inline-block") { "Components" }
              # Link(href: root_path("default"), variant: :ghost, class: "hidden md:inline-block") { "Themes" }
            end
      ...
      def twitter_link
        Link(href: "https://twitter.com/sethhorsley", variant: :ghost, icon: true) do
          svg(
            viewbox: "0 0 20 20",
            aria_hidden: "true",
            fill: "currentColor",
      ...
      def github_link
        Link(href: "https://github.com/sethhorsley/seth_ui", variant: :ghost, icon: true) do
          github_icon
        end
      end
