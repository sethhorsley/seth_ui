...
module SethUI
  class Codeblock < Base
    ...
    def with_clipboard
      SethUI.Clipboard(success: @clipboard_success, error: @clipboard_error, class: "relative") do
        SethUI.ClipboardSource do
          codeblock
        end
        div(class: "absolute top-2 right-2") do
          SethUI.ClipboardTrigger do
            SethUI.Button(variant: :ghost, size: :sm, icon: true, class: "text-white hover:text-white hover:bg-white/20") { clipboard_icon }
          end
        end
      end
    end
